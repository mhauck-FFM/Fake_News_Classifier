...
However, where there is light, there must always be shadow. Misleading, manipulated or false connections can become a serious problem if not clearly recognized as such. Yet, the classification of these posts is not as simple for the user as it seems. In some cases, even the separation of satire and true news can indeed be a tricky task. What a time to be alive!
...
The next step is the one that was extremely new for me and took a whole lot of time to complete. But, getting it done and seeing your app for the first time publicly available is so satisfying. And I learned many useful new things along the way. So, don't be afraid. We are going to deploy the app using AWS Elastic Container Service on Fargate Serverless. That is different from a deployment on a single dedicated EC2 instance, as we don't have to care about setting up the infrastructure by hand and especially the monitoring of the instance(s). We could even use spot instances of EC2 to save some costs. Fargate uses will use a so-called Application Load Balancer (ALB) in our case to spin up the EC2 instance(s) and handle all security and monitoring settings. If more computing power is necessary, the ALB and its autoscaler will take care of it. For the ALB, we need to setup an ECS Service and Task. The ECS Service then requires our Docker container as an image. That sounds like a lot of work, but I'm using a neat toolkit to deploy all that stuff using just python (and linux): the AWS Cloud Development Kit (CDK) - also commonly known as Infrastructure as Code (IaC).

CDK creates a stack in which you specify the AWS VPC, Fargate Service, and Task and the code sets everything up - ready to go for you. Just look at this awesome [tutorial](https://aws-blog.de/2020/03/building-a-fargate-based-container-app-with-cognito-authentication.html). I'm not going to show some code here, as I used many code snippets of the tutorial and adapted them to fit my needs. Also, it really helps to get your hands on it to understand what happens with your code. Note that CDK just "translates" your code into something understandable for CloudFormation, which then does the rest.

That's it! Our app now runs in the cloud and can be accessed using the public IP address of the Load Balancer. Theoretically, we could assign a specific fancy domain (e.g., www.reddifier.com) for the app and route it to the ALB IP using Route 53. And even more fancy, we can use AWS Cognito to implement user authentication. But, that is just extra stuff and of course charged separately by AWS.
